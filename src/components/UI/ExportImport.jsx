  ...
  return (
    <div className="flex gap-3">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm bg-white border border-[#eef1f6] hover:bg-[#f8f9fd] text-[#636e72] transition-all"
      >
        <Download size={16} />
        Export Data
      </button>
      ...
      {showImport && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl border border-[#eef1f6] shadow-2xl w-full max-w-md p-6 animate-scale-in">
            <h3 className="text-lg font-semibold text-[#2d3436] mb-2">Import Backup</h3>
            <p className="text-sm text-[#636e72] mb-4">
              Select a Learning OS backup file (.json) to import your data.
            </p>
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-[#eef1f6] rounded-xl cursor-pointer hover:border-[#5b5fc7] hover:bg-[#f8f9fd] transition-all">
              <FileJson size={32} className="text-[#b2bec3] mb-2" />
              <span className="text-sm text-[#636e72]">Click to select file</span>
              <input type="file" accept=".json" onChange={handleImport} className="hidden" />
            </label>
            <button
              onClick={() => setShowImport(false)}
              className="w-full mt-4 px-4 py-2.5 rounded-xl font-medium text-sm bg-white border border-[#eef1f6] hover:bg-[#f8f9fd] text-[#636e72] transition-all"
            >
              Cancel
            </button>
          </div>
