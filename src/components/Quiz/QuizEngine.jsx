        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <button
              ...
              className="flex items-center gap-1 text-[#64748b] hover:text-[#5b5fc7] transition-colors"
            >
          ...
          <div className="w-full bg-[#eef1f6] rounded-full h-2 mb-2">
            <div
              className="bg-[#5b5fc7] h-2 rounded-full transition-all"
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#eef1f6] mb-6">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-xl border-2 transition-all ${borderClass} ${bgClass} ${
                      !submitted
                        ? "hover:bg-[#f0f2f8] cursor-pointer"
                        : "cursor-default"
                    }`}
                  >
            ...
            {submitted && q.explanation && (
              <div className="mt-4 p-3 rounded-xl bg-[#f8f9fd] border border-[#eef1f6]">
                <p className="text-[#64748b] text-sm">{q.explanation}</p>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              disabled={currentQ === 0}
              className="flex items-center gap-1 px-4 py-2 rounded-xl bg-white border border-[#eef1f6] text-[#1e293b] hover:bg-[#f0f2f8] transition-colors disabled:opacity-40 disabled:cursor-not-allowed text-sm font-medium"
            >
            ...
            {currentQ === activeQuiz.questions.length - 1 && !submitted ? (
              <button
                onClick={submitQuiz}
                className="flex items-center gap-1 px-6 py-2 rounded-xl bg-[#5b5fc7] text-white hover:bg-[#4a4eb5] transition-colors text-sm font-medium"
              >
  ...
  if (view === "results" && activeQuiz) {
    const getGrade = (pct) => {
      ...
      if (pct >= 80)
        return { label: "A", color: "bg-emerald-100 text-emerald-700" };
      if (pct >= 70)
        return { label: "B", color: "bg-[#e8e9f8] text-[#5b5fc7]" };
      if (pct >= 60)
    ...
    return (
      <div className="space-y-6">
        <div>
          <div className="bg-white rounded-2xl p-8 border border-[#eef1f6] text-center mb-6">
            <Trophy size={48} className="text-amber-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-[#1e293b] mb-2">
              Quiz Complete!
            </h2>
            <div className="text-5xl font-bold text-[#5b5fc7] mb-2">
              {score}/{activeQuiz.questions.length}
            </div>
            <p className="text-[#64748b] text-lg">{accuracy}% Accuracy</p>
            <div className="mt-3">
              <span
          ...
          <div className="flex gap-3 justify-center">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 rounded-xl bg-[#5b5fc7] text-white hover:bg-[#4a4eb5] transition-colors text-sm font-medium"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 rounded-xl bg-white border border-[#eef1f6] text-[#1e293b] hover:bg-[#f0f2f8] transition-colors text-sm font-medium"
            >
              Back to Quizzes
            </button>
          </div>
  ...
  if (showCreate) {
    return (
      <div className="space-y-6">
        <div>
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-bold text-[#1e293b]">Create Quiz</h1>
            <button
              onClick={() => {
                setShowCreate(false);
                setView("list");
              }}
              className="flex items-center gap-1 text-[#64748b] hover:text-[#5b5fc7] transition-colors text-sm"
            >
          ...
          <div className="bg-white rounded-2xl p-6 border border-[#eef1f6] mb-6">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#5b5fc7] text-white hover:bg-[#4a4eb5] transition-all text-sm font-medium"
              >
            ...
            <div className="bg-white rounded-2xl p-6 border border-[#eef1f6] mb-6">
              ...
              <div className="space-y-3">
                {newQuiz.questions.map((q, i) => (
                  <div
                    key={q.id}
                    className="flex items-center gap-3 p-3 rounded-xl bg-[#f8f9fd] border border-[#eef1f6]"
                  >
          ...
          <button
            ...
            className="w-full py-3 rounded-xl bg-[#5b5fc7] text-white font-medium hover:bg-[#4a4eb5] transition-all text-sm disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Save Quiz
          </button>
        </div>
      ...
      <div>
        <div className="flex items-start justify-between mb-8 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#e8e9f8] flex items-center justify-center">
              <Brain size={22} className="text-[#5b5fc7]" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-[#1e293b]">Quiz Engine</h1>
        ...
        {quizzes.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 border border-[#eef1f6] text-center">
            <Brain size={48} className="text-[#c5c7e8] mx-auto mb-4" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {quizzes.map((quiz) => {
              ...
              return (
                <div
                  key={quiz.id}
                  className="bg-white rounded-2xl p-6 border border-[#eef1f6] hover:shadow-md transition-shadow"
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-[#e8e9f8] text-[#5b5fc7] font-medium">
                  ...
                  <button
                    ...
                    className="w-full py-2 rounded-xl bg-[#f0f2f8] text-[#5b5fc7] hover:bg-[#e8e9f8] transition-colors text-sm font-medium"
                  >
                    Take Quiz
                  </button>
                </div>
